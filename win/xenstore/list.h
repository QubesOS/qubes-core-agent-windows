 ...
 * This is useful for implementing queues.
 */
static __inline void list_add_tail_rcu(struct list_head *new, struct list_head *head)
{
...
static __inline int hlist_unhashed(struct hlist_node *h) 
{ 
	return !h->pprev;
} 

static __inline int hlist_empty(struct hlist_head *h) 
{ 
	return !h->first;
} 
...
static __inline void hlist_add_head_rcu(struct hlist_node *n, struct hlist_head *h) 
{ 
	struct hlist_node *first = h->first;
	n->next = first;
	n->pprev = &h->first; 
...
/* next must be != NULL */
static __inline void hlist_add_before(struct hlist_node *n, struct hlist_node *next)
{
	n->pprev = next->pprev;
	n->next = next; 
	next->pprev = &n->next; 
